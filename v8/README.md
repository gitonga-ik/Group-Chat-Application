# Group Chat System (V8)

A concurrent, multi-user group chat application built as a multi-threaded UDP server. This version implements a thread-per-request model for connectionless communication.
...
- **Concurrency:** Multi-threaded server using `pthread` to handle each incoming datagram request asynchronously.
...
- **Server Model:** Connectionless multi-threaded server.
